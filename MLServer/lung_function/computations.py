...
def getInputValues(pef_model, fef_model, eng_curve, frm_times):
    ...
    for i in np.arange(eng_curve.size - 1):
        total_eng = total_eng + computeVolume(eng_curve[i], eng_curve[i + 1], delta_t)
    ...
    times = np.array([0, frm_times[i_25], frm_times[i_max],
                        frm_times[i_50], frm_times[i_75], frm_times[frm_times.size - 1]])
    ...
    for i in np.arange(times.size - 1):
        total_volume = total_volume + \
            computeVolume(
                flow_rates[i], flow_rates[i + 1], times[i + 1] - times[i])

    return max_eng, total_volume

def getOutputValues(pef_model, fef_model, fvc_model, fev1_model, eng_curve, frm_times):
    eng_curve = np.array(eng_curve)
    frm_times = np.array(frm_times)
    max_eng, total_volume = getInputValues(
        pef_model, fef_model, eng_curve, frm_times)
